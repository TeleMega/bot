...
function show_supergroup_settingsmod(msg, target)
  ...
  local text = "<code>#-SuperGroup settings</code>>>>\n\n<code>#-Lock Links-#</code> <b>|"..settings.lock_link.."|</b>\n<code>#-Lock Webpage-#</code> <b>|"..settings.lock_webpage.."|</b>\n<code>#-Lock Tag-#</code> <b>|"..settings.lock_tag.."|</b>\n<code>#-Lock Emoji-#</code> <b>|"..settings.lock_emoji.."|</b>\n<code>#-Lock English-#</code> <b>|"..settings.lock_eng.."|</b>\n<code>#-Lock Badword-#</code> <b>|"..settings.lock_badw.."|</b>\n<code>#-Lock Flood-#</code> <b>|"..settings.flood.."|</b>\n<code>#-Flood sensitivity-#</code> <b>|"..NUM_MSG_MAX.."|</b>\n<code>#-Lock Spam</code> <b>|"..settings.lock_spam.."|</b>\n<code>#-Lock Contacts-#</code> <b>|"..settings.lock_contacts.."|</b>\n<code>#-Lock Arabic/Persian-#</code> <b>|"..settings.lock_arabic.."|</b>\n<code>#-Lock Member-#</code> <b>|"..settings.lock_member.."|</b>\n<code>#-Lock RTL-#</code> <b>|"..settings.lock_rtl.."|</b>\n<code>#-Lock Forward-#</code> <b>|"..settings.lock_fwd.."|</b>\n<code>#-Lock TGservice-#</code> <b>|"..settings.lock_tgservice.."|</b>\n<code>#-Lock Sticker-#</code> <b>|"..settings.lock_sticker.."|</b>\n<code>#-Public-#</code> <b>|"..settings.public.."|</b>\n<code>#-Strict Settings-#</code> <b>|"..settings.strict.."|</b>\n"
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'id' then
			...
			elseif msg.text:match("@[%a%d]") then
				...
				return reply_msg(msg.id, "<b>#SuperGroup ID</b>: <code>"..msg.to.id.."</code>\n<b>#SuperGroup Name</b>: <code>"..msg.to.title.."</code>\n<b>#First Name</b>: <code>"..(msg.from.first_name or '404 not found').."</code>\n<b>#Last Name</b>: <code>"..(msg.from.last_name or '404 not found').."</code>\n<b>#Your ID</b>: <code>"..msg.from.id.."</code>\n<b>#Username:</b> <code>@"..(msg.from.username or '404 not found').."</code>\n<b>#Phone Number</b>: <code>+"..(msg.from.phone or '404 Not Found').."</code>\n<b>Group Type</b>: <code>#SuperGroup</code>", ok_cb, false)		end
		end
		...
		if matches[1] == 'newlink' and is_momod(msg)then
			...
			local receiver = get_receiver(msg)
				if success == 0 then
					send_large_msg(receiver, '<i>Error</i> \n<code>Reason: Not creator</code> \n <code>please use</code> <b>/setlink</b> <code>to set it</code>')
		...
		if matches[1] == 'setlink' and is_owner(msg) then
			...
			save_data(_config.moderation.data, data)
			return '<code>Please send the new group</code> <b>link</b> <code>now!</code>'
		end
		...
		if matches[1] == 'link' then
			...
			if not group_link then
				return "<code>#Create a link using</code> <b>/newlink</b> <code>first!</code>\n\n<code>Or if I am not creator use</code> <b>/setlink</b> <code>to set your link</code>"
			end
		...
		if matches[1] == 'setxadmin' and is_owner(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'setowner',
					msg = msg
			...
			elseif matches[1] == 'setxadmin' and string.match(matches[2], '^%d+$') then
			local group_owner = data[tostring(msg.to.id)]['set_owner']
				if group_owner then
					local receiver = get_receiver(msg)
				...
				local user_id = matches[2]
				channel_get_users (receiver, in_channel_cb, {get_cmd=get_cmd, receiver=receiver, msg=msg, user_id=user_id})
			elseif matches[1] == 'setxadmin' and not string.match(matches[2], '^%d+$') then
				local	get_cmd = 'setowner'
				local	msg = msg
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
		...
		if matches[1] == 'modprom' then
		  if not is_momod(msg) then
			...
			elseif matches[1] == 'modprom' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'modprom'
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] promoted user#id"..matches[2])
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == 'modprom' and not string.match(matches[2], '^%d+$') then
				local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'modprom',
				}
		...
		if matches[1] == 'moddem' then
			if not is_momod(msg) then
				return
			end
			if not is_owner(msg) then
				return reply_msg(msg.id,"<code>Error</code> \n<b>Only owner/support/admin can promote</b>",ok_cb,false)
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'moddem',
			...
			elseif matches[1] == 'moddem' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
